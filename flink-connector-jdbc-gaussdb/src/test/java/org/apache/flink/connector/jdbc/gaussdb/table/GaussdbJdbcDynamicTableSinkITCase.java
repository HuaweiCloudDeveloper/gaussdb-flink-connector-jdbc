...
public class GaussdbJdbcDynamicTableSinkITCase extends AbstractTestBase implements GaussdbTestBase {
    ...
    protected final TableRow ignoreNullUpdateTable = createIgnoreNullUpdateTable();
    ...
    protected TableRow createIgnoreNullUpdateTable() {
        return tableRow(
                "user_sink",
    ...
    public List<TableManaged> getManagedTables() {
        return Arrays.asList(
                ...
                userOutputTable,
                ignoreNullUpdateTable);
    }

    @BeforeEach
    void beforeEach() throws SQLException {
        try (Connection conn = getMetadata().getConnection()) {
            realOutputTable.createTable(conn);
            upsertOutputTable.createTable(conn);
            appendOutputTable.createTable(conn);
            batchOutputTable.createTable(conn);
            checkpointOutputTable.createTable(conn);
            userOutputTable.createTable(conn);
            ignoreNullUpdateTable.createTable(conn);
        }
    }
    ...
    void afterEach() throws SQLException {
        ...
        try (Connection conn = getMetadata().getConnection()) {
            realOutputTable.dropTable(conn);
            upsertOutputTable.dropTable(conn);
            appendOutputTable.dropTable(conn);
            batchOutputTable.dropTable(conn);
            checkpointOutputTable.dropTable(conn);
            userOutputTable.dropTable(conn);
            ignoreNullUpdateTable.dropTable(conn);
        }
    }
    ...
    @Test
    public void testIgnoreNullWhenUpdateEnabled() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.getConfig().enableObjectReuse();
        env.getConfig().setParallelism(1);
        StreamTableEnvironment tEnv = StreamTableEnvironment.create(env);
        ...
        String tableName = "user_sink";
        tEnv.executeSql(
                ignoreNullUpdateTable.getCreateQueryForFlink(
                        getMetadata(),
                        tableName,
                        Arrays.asList(
                                "'connector' = 'gaussdb'",
        ...
        tEnv.executeSql(String.format("INSERT INTO %s VALUES (1, 'John Doe', 25)", tableName))
                .await();

        // Update with null values - should ignore null fields
        tEnv.executeSql(
                        String.format(
                                "INSERT INTO %s VALUES (1, CAST(NULL AS VARCHAR(100)), 30)",
                                tableName))
                .await();

        List<Row> results = ignoreNullUpdateTable.selectAllTable(getMetadata());
        assertThat(results).hasSize(1);
        // Email should remain unchanged (not null) because ignoreNullWhenUpdate=true
        assertThat(results.get(0)).isEqualTo(Row.of(1, "John Doe", 30));
    }

    @Test
    public void testIgnoreNullWhenUpdateDisabled() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.getConfig().enableObjectReuse();
        env.getConfig().setParallelism(1);
        StreamTableEnvironment tEnv = StreamTableEnvironment.create(env);
        ...
        tEnv.executeSql(
                ignoreNullUpdateTable.getCreateQueryForFlink(
                        ...
                        Arrays.asList(
                                "'connector'='gaussdb'",
                                "'sink.ignore-null-when-update' = 'false'",
                                "'sink.buffer-flush.max-rows' = '1'",
                                "'sink.buffer-flush.interval' = '0'")));

        // Insert initial record
        tEnv.executeSql(String.format("INSERT INTO %s VALUES (1, 'Jane Doe', 28)", tableName))
                .await();

        // Update with null values - should update null fields
        tEnv.executeSql(
                        String.format(
                                "INSERT INTO %s VALUES (1, CAST(NULL AS VARCHAR(100)), 32)",
                                tableName))
                .await();

        List<Row> results = ignoreNullUpdateTable.selectAllTable(getMetadata());
        assertThat(results).hasSize(1);
        // Email should be null because ignoreNullWhenUpdate=false
        assertThat(results.get(0)).isEqualTo(Row.of(1, null, 32));
    }
}
