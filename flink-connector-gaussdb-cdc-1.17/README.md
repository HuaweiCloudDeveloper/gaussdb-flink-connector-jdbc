...
修改 `flink-connector-gaussdb-cdc-1.17/pom.xml`，将 `gaussdbjdbc` 依赖的 scope 改为 `provided`：
...
```bash
mvn clean package -pl flink-connector-gaussdb-cdc-1.17 -am -DskipTests
```
...
| 方式 | JAR 大小 | 内置驱动 | 适用场景 |
|------|---------|---------|---------|
| fat JAR（默认） | ~2 MB | ✅ 内置 | 推荐，部署简单 |
| thin JAR | ~100 KB | ❌ 不包含 | 已有驱动管理规范，需灵活切换驱动版本 |
